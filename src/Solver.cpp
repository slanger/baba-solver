		...
		if (!winning_state)
		{
			...
			std::for_each(std::execution::par, parallelism_roots.begin(), parallelism_roots.end(),
				[&options, &result, &mutex, &seen_states, &winning_state, &best_leaf_states, &next_thread_id, &num_threads_finished, &total_num_threads](std::shared_ptr<GameState> state)
				{
					...
					std::unordered_map<std::shared_ptr<GameState>, uint8_t, GameStatePtrHasher, GameStatePtrComparer> local_seen_states = seen_states;
					...
					while (!stack.empty())
					{
						...
						if (turn_count <= options.max_cache_depth)
						{
							...
							auto [iter, inserted] = local_seen_states.insert({ new_state, turn_count });
							if (!inserted)
							{
								if (turn_count >= iter->second)
								{
