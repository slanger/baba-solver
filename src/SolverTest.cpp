...
class GameStateTest : public BabaSolver::GameState {
 public:
  ...
  uint8_t TurnCount() const override { return _turn; }

  std::shared_ptr<GameState> ApplyMove(
      BabaSolver::Direction direction) const override {
    std::shared_ptr<GameStateTest> new_state =
        std::make_shared<GameStateTest>(*this);
    int8_t i = _baba.i;
    int8_t j = _baba.j;
    switch (direction) {
      ...
      case BabaSolver::Direction::RIGHT:
        ...
        break;
      default:
        std::cerr << "Invalid Direction: " << static_cast<uint8_t>(direction)
                  << std::endl;
        break;
    }
    new_state->_turn += 1;
  ...
  int CalculateScore() const override {
    ...
    int distance = std::abs(_baba.i - (GRID_HEIGHT - 1)) +
                   std::abs(_baba.j - (GRID_WIDTH - 1));
    return max_distance - distance;
  }

  void ResetContext() override { _turn = 0; }

  // Not implemented.
  std::vector<BabaSolver::Direction> Moves() const override {
    return std::vector<BabaSolver::Direction>();
  }
...
TEST(SolverTest, FindsSolution) {
  ...
  BabaSolver::SolverResult result =
      BabaSolver::Solve("Test Level", initial_state, options);
  EXPECT_TRUE(result.solved);
  ASSERT_EQ(result.iterations.size(), 1);
  const std::shared_ptr<BabaSolver::GameState>& end_state =
      result.iterations.back().end_state;
