...
constexpr GameObject ALWAYS_MOVABLE_OBJECTS[] = {
    GameObject::KEY, GameObject::ROCK_TEXT, GameObject::IS_TEXT,
...
GameStateMtnE1::GameStateMtnE1()
  ...
  AddToCellInPlace(_grid[0][7], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[0][8], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[0][9], GameObject::IMMOVABLE);
  ...
  AddToCellInPlace(_grid[15][15], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[15][16], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[15][17], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[16][15], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[16][16], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[16][17], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[17][15], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[17][16], GameObject::IMMOVABLE);
  AddToCellInPlace(_grid[17][17], GameObject::IMMOVABLE);
  // Add the other game objects to the grid.
  AddToCellInPlace(_grid[4][3], GameObject::ROCK);
  AddToCellInPlace(_grid[6][5], GameObject::ROCK);
  AddToCellInPlace(_grid[6][11], GameObject::ROCK);
  AddToCellInPlace(_grid[4][11], GameObject::ROCK_TEXT);
  AddToCellInPlace(_grid[_is_text.i][_is_text.j], GameObject::IS_TEXT);
  AddToCellInPlace(_grid[4][13], GameObject::PUSH_TEXT);
  AddToCellInPlace(_grid[DOOR_I][DOOR_J], GameObject::DOOR);
...
uint8_t GameStateMtnE1::TurnCount() const { return _turn; }

std::vector<Direction> GameStateMtnE1::Moves() const {
  return std::vector<Direction>(_moves, _moves + _turn);
}

std::shared_ptr<GameState> GameStateMtnE1::ApplyMove(
    Direction direction) const {
  std::shared_ptr<GameStateMtnE1> new_state =
      std::make_shared<GameStateMtnE1>(*this);
...
bool GameStateMtnE1::HaveWon() const {
  // Optimization: The Door's coordinates are hard-coded because it doesn't
  // move.
  return CellContainsGameObject(_grid[DOOR_I][DOOR_J], GameObject::KEY);
}

bool GameStateMtnE1::CheckIfPossibleToWin() const {
  if (!AllBabasAlive()) return false;

  // If any of the text objects are *not* on the upper right platform or to the
  // right of it, then it's impossible to win.
  if (_is_text.i <= 2 || _is_text.i >= 8 || _is_text.j <= 9) return false;

  for (int8_t i = 0; i <= 2; ++i) {
    for (int8_t j = 10; j <= 17; ++j) {
      if (CellContainsGameObject(_grid[i][j], GameObject::ROCK_TEXT) ||
          CellContainsGameObject(_grid[i][j], GameObject::PUSH_TEXT))
        return false;
    }
  }
...
int GameStateMtnE1::CalculateScore() const {
  if (!CheckIfPossibleToWin()) return -1'000'000;

  // First milestone: The three rocks are moved in between the two upper
  // platforms. This creates a "bridge" between the two platforms.
  int score = 0;
  int8_t rock_row = -1;
  for (int8_t i = 3; i <= 7; ++i) {
    int rock_count = 0;
    for (int8_t j = 7; j <= 9; ++j) {
      if (CellContainsGameObject(_grid[i][j], GameObject::ROCK)) ++rock_count;
    }
    switch (rock_count) {
      case 1:
        score += 100;
  ...
  // Second milestone: The text blocks are moved to the right of the upper right
  // platform and are aligned with the rocks that are in between the upper
  // platforms. This lets the player move the Babas closer together (over the
  // rocks and stopped on the right by the text blocks).
  if (rock_row != -1) {
    if (!CheckIfTextCanBeAlignedWithRocks(rock_row)) return -1;

    int text_aligned_count = 0;
    if (_is_text.i == rock_row) ++text_aligned_count;
    for (int8_t j = 10; j <= 17; ++j) {
      if (CellContainsGameObject(_grid[rock_row][j], GameObject::ROCK_TEXT) ||
          CellContainsGameObject(_grid[rock_row][j], GameObject::PUSH_TEXT))
        ++text_aligned_count;
    }
  ...
  // Third milestone: Both Babas are on the same space. After this point, the
  // player can move the Babas freely and move the key to the door.
  if (BabasOnSameSpace()) {
    score += 10'000'000;
  }
...
void GameStateMtnE1::PrintGrid() const {
...
void GameStateMtnE1::MoveBaba(Coordinate& baba, Direction direction) {
...
bool GameStateMtnE1::CheckCellAndMoveObjects(int8_t i, int8_t j, int8_t delta_i,
                                             int8_t delta_j,
                                             uint16_t prev_cell) {
  ...
  if (CellContainsGameObject(cell, GameObject::DOOR)) {
    if (CellContainsGameObject(prev_cell, GameObject::KEY)) {
      uint16_t prev_cell_without_key =
          RemoveFromCell(prev_cell, GameObject::KEY);
      if (!CellContainsMovableObject(prev_cell_without_key,
                                     _rock_is_push_active))
        return true;
    }
    return false;
  }
...
void GameStateMtnE1::RecalculateState() {
...
bool GameStateMtnE1::CheckIfTextCanBeAlignedWithRocks(int8_t rock_row) const {
  if (_is_text.i != rock_row && _is_text.j >= 15) return false;
  if (_is_text.j >= 15 && _rock_is_push_active) return false;

  for (int8_t i = 3; i <= 7; ++i) {
    if (i == rock_row) continue;
...
std::size_t GameStateMtnE1::Hash() const {
