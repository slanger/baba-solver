...
// GameState is a pure virtual interface for the object that stores and
...
class GameState {
 public:
  ...
  // Compares this GameState object with another, and returns true if they're
  // equal.
  ...
  // Applies the given move to the current state and returns the resulting
  // GameState. Does *not* modify this GameState.
  ...
  // Returns true if it's possible to reach a winning state from this GameState,
  // false otherwise.
  virtual bool CheckIfPossibleToWin() const = 0;

  // Calculates the "score" of this GameState, which represents how likely the
  // GameState will lead a winning game state.
...
// Function object for hashing GameStates (inside std::shared_ptr's) for a hash
// map/set.
struct GameStatePtrHasher {
  std::size_t operator()(const std::shared_ptr<GameState>& state) const;
};

// Function object for comparing GameStates (inside std::shared_ptr's) for
// equality.
struct GameStatePtrComparer {
  bool operator()(const std::shared_ptr<GameState>& lhs,
